...
const CashYear = ({ navigation, route }) => {
  ...
  const handleYearSelect = (year) => {
    navigation.navigate("CashBank", { brand, model, category, year })
    console.log("Selected:", brand, model, category, year)
  }
    ...
    <SafeAreaView className="flex-1 bg-brand-light mt-2.5">
      <View className="bg-white rounded-xl shadow-md overflow-hidden mx-4">
        ...
        <View className="bg-gray-100 px-4 pt-4 pb-2 mt-2">
          ...
          {/* Step Labels (aligned under bubbles) */}
          <View className="flex-row justify-between items-center mt-1">
            {["Choose brand", "Select model", "Select category", "Select year"].map((label, index) => (

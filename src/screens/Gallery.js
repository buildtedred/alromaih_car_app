...
import { View, Text, Image, ScrollView, TouchableOpacity, FlatList, Dimensions, Animated } from "react-native"
...
// Import the same modals used in car-discover-screen.js
...
export default function GalleryScreen({ route }) {
  ...
  // Add state for modals visibility
  ...
  useEffect(() => {
    ...
    const similar = carsData
      .filter((c) => c.id !== car.id && (c.brand === car.brand || Math.abs(c.cashPrice - car.cashPrice) < 20000))
      .slice(0, 4)
  ...
  const handleMethodSelection = (method) => {
    ...
    if (method === "cash") {
      console.log("Opening cash modal from gallery screen")
      setCashModalVisible(true)
    } else if (method === "finance") {
      console.log("Opening finance modal from gallery screen")
          ...
          <View className="w-1/2 max-w-[180px] h-[180px] bg-white rounded-2xl border-2 border-[#46194F] p-2 justify-between">
            <View>
              ...
              <View className="flex-row bg-white rounded-3xl overflow-hidden border border-[#46194F] mb-2">
                ...
                <TouchableOpacity
                  onPress={() => handleMethodSelection("finance")}
            ...
            <TouchableOpacity
              className="bg-[#46194F] py-1.5 rounded-xl"
              onPress={() => handleMethodSelection(selectedMethod)}
            >

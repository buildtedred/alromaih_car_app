...
export default function GalleryScreen({ route }) {
  ...
  const [selectedColorIndex, setSelectedColorIndex] = useState(1) // Default to second color
  ...
  const colors = [
    { code: "#9E2B11", name: "أحمر" },
    { code: "#A9B4BC", name: "رمادي سماوي" },
    { code: "#7D6C55", name: "بني" },
    { code: "#1D2327", name: "أسود" },
    { code: "#E9E56F", name: "أصفر" },
    { code: "#5D7A8C", name: "أزرق" },
    { code: "#4F5A64", name: "رمادي داكن" },
  ];

  // Set default color on component mount
  useEffect(() => {
    if (!selectedColor) {
      // Set the second color (رمادي سماوي) as default
      handleColorSelect(colors[1].code, colors[1].name, 1);
    }
  }, []);

  const handleColorSelect = (colorCode, colorName, index) => {
  ...
  useEffect(() => {
    setCurrentImages(activeTab === "interior" ? fullCar.interiorImages : fullCar.exteriorImages)
  }, [activeTab, fullCar])
        ...
        <Animated.FlatList
          ...
          renderItem={({ item }) => (
            <View style={{ width }} className="items-center justify-center ">
              <View
                className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm pl-4 px-4"
        ...
        <View className="w-1/2 max-w-[180px] h-[180px] bg-white rounded-2xl border-2 border-[#46194F] p-2 justify-between">
          <View>
            ...
            <View className="flex-row bg-white rounded-3xl overflow-hidden border border-[#46194F] mb-2">
              ...
              >
                <Text
                  className={`text-center text-[11px] font-bold ${selectedMethod === "cash" ? "text-white" : "text-[#46194F]"}`}
                >
                  كاش
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => {
                  setSelectedMethod("finance")
                  setFinanceModalVisible(true)
                }}
                className={`flex-1 ${selectedMethod === "finance" ? "bg-[#46194F]" : ""}`}
              >
                <Text
                  className={`text-center text-[11px] font-bold ${selectedMethod === "finance" ? "text-white" : "text-[#46194F]"}`}
                >
                  التمويل
                </Text>
              </TouchableOpacity>
        ...
        <View className="w-1/2 max-w-[180px] h-[180px] bg-white rounded-2xl border-2 border-[#46194F] p-2 justify-between">
          <View>
            <View className="flex-row-reverse justify-between items-end mb-2">
              ...
              <View className="items-start w-[100px]">
                <Text
                  numberOfLines={1}
                  ellipsizeMode="tail"
                  className="text-[10px] font-bold text-[#46194F] truncate w-full"
                >
                  جيتور T2
                </Text>
                <Text className="text-xs text-gray-500 mt-0.5 w-full">{limitWords("لدكري فل كامل 2025", 2)}</Text>
              </View>
            </View>
            ...
            <View className="items-start">
              ...
              {/* Updated color selection UI with box appearing at color position */}
              <View className="flex-row-reverse items-center">
                
                {/* Color options with pill appearing at selected color */}
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                ...
                >
                  {colors.map((color, idx) => (
                    <View key={idx} className="relative">
                      {/* If this is the selected color, show the pill */}
                      {selectedColor === color.code ? (
                        <View
                          className="flex-row-reverse items-center px-3 py-1 rounded-full border border-gray-300"
                          style={{ backgroundColor: "white" }}
                        >
                          <Text className="text-[#6A7D8B] text-xs mr-1">{color.name}</Text>
                          <View className="h-4 w-4 rounded-full" style={{ backgroundColor: color.code }} />
                        </View>
                      ) : (
                        /* Otherwise show the color circle */
                        <TouchableOpacity
                          onPress={() => handleColorSelect(color.code, color.name, idx)}
                          className="w-5 h-5 rounded-full border-2 border-transparent mx-1"
      ...
      <View className="px-4 mt-6 mb-4">
        ...
        <View className="flex-row justify-between">
          {[
            ...
            <View key={idx} className="bg-white border border-[#E5E5E5] rounded-xl px-3 py-2 items-center flex-1 mx-1">
            ...
            {expanded === section.key && section.data && section.data.length > 0 && (
              <View className="bg-white px-4 pb-4">
                {section.data.map((item, index) => (
                  <View key={index} className="flex-row justify-between items-start py-2 border-b border-[#46194F]">
